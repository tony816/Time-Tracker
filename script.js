...
class TimeTracker {
    constructor() {
        ...
        // Track the last saved snapshot signature.
    ...
    init() {
        ...
        this.setSaveStatus('idle', '\uB300\uAE30 \uC911');
        this.setSyncStatus('idle', '\uB3D9\uAE30\uD654 \uB300\uAE30 \uC911');
        ...
        // ?Ä?•ÏÜå ?ÑÏ≤¥???®ÏïÑ?àÏùÑ ???àÎäî legacy outcome ?ÑÎìú ?ºÍ¥Ñ ?úÍ±∞
        try {
            setTimeout(() => { this.purgeOutcomeFromAllStoredData(); }, 0);
        } catch (_) {}

        // Studio ???ÑÌôò ?±ÏúºÎ°?hidden?????Ä?¥Î®∏ ?§Î°ú?ÄÎßÅÏùÑ ?ºÌïòÍ≥?Î∂àÌïÑ?îÌïú ?∏Î¶¨Í±∞Î? Ï§ÑÏûÑ
    ...
    attachEventListeners() {
        if (this.authButton) {
            this.authButton.addEventListener('click', () => {
                if (!this.supabaseConfigured || !this.supabase) {
                    this.showNotification('Supabase ?§Ï†ï??Î®ºÏ? ?ïÏù∏?¥Ï£º?∏Ïöî.');
                    return;
                }
                if (this.supabaseUser) {
                    ...
                    this.saveData().finally(() => {
                        this.supabase.auth.signOut().catch((err) => {
                            ...
                            this.showNotification('Î°úÍ∑∏?ÑÏõÉ Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.');
                        });
                    });
                } else {
                    ...
                    this.supabase.auth.signInWithOAuth(params).catch((err) => {
                        ...
                        this.showNotification('Google Î°úÍ∑∏?∏Ïóê ?§Ìå®?àÏäµ?àÎã§.');
                    });
                }
            });
        }
        ...
        // Î≥ëÌï©???Ä?Ä ?ÑÏ†Ñ ?ºÏ≤¥?? ?¥Îäê ?ÑÏπòÎ•??¥Î¶≠?¥ÎèÑ ?ÑÏ≤¥ Î≤îÏúÑ ?†ÌÉù
        const timeEntries = document.getElementById('timeEntries');
        if (timeEntries) {
            // Ï∫°Ï≤ò ?®Í≥Ñ?êÏÑú Î®ºÏ? Í∞ÄÎ°úÏ±î: ?úÎûòÍ∑??®Ïùº?†ÌÉù Î°úÏßÅÎ≥¥Îã§ ?†Ìñâ
            ...
            // Ï¢åÏ∏°???∏Î≤ÑÎßåÏúºÎ°úÎèÑ ?§Ï?Ï§?Î≤ÑÌäº???êÏó∞?§ÎüΩÍ≤??∞Îùº?§ÎèÑÎ°?ÎßàÏö∞???¥Îèô Ï∂îÏ†Å
            ...
            // Í≥ÑÌöç ?ÖÎ†• ?ÑÎìú ?¥Î¶≠ ???úÎ°≠?§Ïö¥ Î∞îÎ°ú ?¥Í∏∞
        ...
        // Í∞Ä?∏Ïò§Í∏??¥Î≥¥?¥Í∏∞ Í∏∞Îä• ?úÍ±∞?? Í¥Ä??UI Î∞?Î¶¨Ïä§???ÜÏùå
        ...
        // ?Ä?¥Î®∏ Í≤∞Í≥º ?ÖÎ†• ?ÑÎìú ?¥Î≤§??Î¶¨Ïä§??(?∞Ï∏° Ïπ∏Í≥º Î™®Îã¨???∞Í≤∞: Î≥ëÌï© ?¨Ìï® Í∞±Ïã†)
        document.getElementById('timeEntries').addEventListener('input', (e) => {
        ...
        // ?úÍ? IME ???ÖÎ†• Ï°∞Ìï© Ï¢ÖÎ£å ???Ä??Î≥¥Ï°∞(?ºÎ? ?òÍ≤Ω?êÏÑú input ?¥Î≤§??ÏßÄ???ÑÎùΩ ?ÄÎπ?
        ...
        // ?¨Ïª§?§Í? Îπ†Ïßà ?åÎèÑ Î≥¥Ï°∞ ?Ä???∏Î¶¨Í±?(blur??Î≤ÑÎ∏îÎß???????focusout ?¨Ïö©)
        ...
        // change ?¥Î≤§??Î≥¥Ï°∞ ?? ?ºÎ? ?òÍ≤Ω?êÏÑú input ?¥Î≤§?∏Í? ?ÑÎùΩ?????àÏùå
    ...
    // Î≥ëÌï© ?Ä ?¥Î? ?¥ÎîîÎ•??¥Î¶≠?¥ÎèÑ ?ÑÏ≤¥ Î≥ëÌï© Î≤îÏúÑÎ•??†ÌÉù?òÎèÑÎ°?Ï∫°Ï≤ò Ï≤òÎ¶¨
    ...
    // Í∞Ä?∏Ïò§Í∏??¥Î≥¥?¥Í∏∞ Í∏∞Îä• ?úÍ±∞?? Í¥Ä???®Ïàò ??†ú
    ...
    // ?úÍ∞Ñ ?ºÎ≤®('00','1'..'23') <-> ?ïÏàò ??0..23) Î≥Ä???¨Ìçº
    labelToHour(label) {
    ...
    // Î©îÎ™®Î¶?-> DB ?ÑÏÜ°??slots JSON ?ùÏÑ±(ÎπÑÏñ¥?àÎäî ?úÍ∞Ñ?Ä ?ùÎûµ)
        buildSlotsJson() {
        return globalThis.TimeTrackerPersistenceController.buildSlotsJson.call(this);
    }
    // DB slots JSON -> Î©îÎ™®Î¶?Î∞òÏòÅ(Ï°¥Ïû¨?òÎäî ?§Îßå Î∞òÏòÅ)
    ...
    // ?Ä?•ÏÜå ?ÑÏ≤¥ ?úÌöå?òÏó¨ Í∏∞Ï°¥ ?∞Ïù¥?∞Ïóê??outcome ?ÑÎìú ?úÍ±∞
    ...
    startFieldSelection(type, index, e) {
        if (type !== 'planned') return; // ?∞Ï∏° ?¥Ïóê???úÏûë Ï°∞Ïûë Í∏àÏ?
    ...
    mergeSelectedFields(type) {
        ...
        if (selectedSet.size > 1) {
            ...
            // Í∏∞Ï°¥ Î≥ëÌï©Í≥??ºÎ?Îß?Í≤πÏπò??Í≤ΩÏö∞?êÎèÑ ?ÑÏ≤¥ Î≥ëÌï© Î∏îÎ°ù ?®ÏúÑÎ°??ïÏû•??orphan Î≥¥Ï°∞ ?¨Î°Ø??Î∞©Ï??úÎã§.
            ...
            // Ï¢åÏ∏° Í≥ÑÌöç ?¥Ïù¥ Î≥ëÌï©????Î™®Îì† ?¥ÏùÑ ?ôÍ∏∞??Î≥ëÌï©
            if (type === 'planned') {
                // Ï§ëÏïô ?úÍ∞Ñ ??Î≥ëÌï© (?úÍ∞Ñ Î≤îÏúÑ ?úÏãú)
                ...
                // ?∞Ï∏° ?§Ï†ú ?úÎèô ??Î≥ëÌï© (Í∏∞Ï°¥ Í∞íÏù¥ ?àÎã§Î©??†Ï?, ?ÜÏúºÎ©?Îπ?Í∞?
    ...
    computeSplitSegments(type, index) {
        ...
        const buildGridSegmentsFromActivities = (activities, totalUnits, options = {}) => {
            return actualGridCore.buildSplitGridSegmentsFromActivities(activities, {
                index,
                baseIndex,
                isMergedRange,
                unitsPerRow,
    ...
    getSplitActivities(type, baseIndex) {
        ...
        // Í≥ÑÌöç Î∂ÑÌï¥Í∞Ä ?àÏúºÎ©?Í≥ÑÌöç ?àÏù¥?ÑÏõÉ Í∑∏Î?Î°??¨Ïö© (?ºÎ≤® ?çÏä§?∏Îßå)
        ...
        // Í≥ÑÌöç???ÜÏùÑ ?åÎäî ?§Ï†ú Î∂ÑÌï¥ÎßåÏúºÎ°?Í∑∏Î¶¨???ùÏÑ±
        return normalizedActual;
    }
    ...
    createTimerField(index, slot) {
        ...
        return `<div class="actual-field-container">
                    ...
                    <button class="activity-log-btn" data-index="${index}" aria-label="?úÎèô ?ÅÏÑ∏ Í∏∞Î°ù ?¥Í∏∞" title="?ÅÏÑ∏ Í∏∞Î°ù ?¥Í∏∞">Í∏∞Î°ù</button>
                </div>`;
    }
    ...
    createMergedTimeField(mergeKey, index, slot) {
        ...
        if (index === start) {
            ...
            // Î≥ëÌï©???úÍ∞Ñ ?ÑÎìú??ÎßàÏ?Îß?Î≥¥Ï°∞ ?Ä - ?òÎã® Í≤ΩÍ≥Ñ???†Ï?
            ...
            // Î≥ëÌï©???úÍ∞Ñ ?ÑÎìú??Ï§ëÍ∞Ñ Î≥¥Ï°∞ ?Ä - ?ÑÏ†Ñ??Í≤ΩÍ≥Ñ???úÍ±∞
    ...
    normalizeDurationStep(seconds) {
        ...
        // Keep the raw duration in whole seconds.
    ...
    formatActivitiesSummary(activities, options = {}) {
        ...
        const normalized = items
            .map((item) => ({
                label: this.normalizeActivityText ? this.normalizeActivityText(item?.label || '') : String(item?.label || '').trim(),
                seconds: this.normalizeDurationStep(Number.isFinite(item?.seconds) ? Number(item.seconds) : 0) || 0,
            }))
            .filter((item) => item.label || item.seconds > 0);
        if (normalized.length === 0) return '';
        const parts = normalized.map((item) => {
            const duration = this.formatDurationSummary(item.seconds);
            return item.label ? `${item.label} ${duration}` : duration;
        });
    ...
    enforceActualLimit(index) {
        ...
        if (secs != null && Number.isFinite(secs) && secs > limit) {
            ...
            this.showNotification('Í∏∞Î°ù ?úÍ∞Ñ?Ä ??Ïπ∏Îãπ ÏµúÎ? 60Î∂ÑÍπåÏßÄ ?ÖÎ†•?????àÏäµ?àÎã§.');
        }
    }
    ...
    createActualTimeControl({ kind, index, seconds, label, disabled = false }) {
        ...
        caption.textContent = kind === 'grid' ? 'Í∏∞Î°ù' : 'Î∞∞Ï†ï';
        ...
        input.readOnly = true;
        input.setAttribute('aria-label', kind === 'grid' ? 'Í∏∞Î°ù ?úÍ∞Ñ' : 'Î∞∞Ï†ï ?úÍ∞Ñ');
    ...
    // ?çÏä§?∏Ïóê???úÍ∞ÑÍ∞?HH:MM(:SS) ?êÎäî 1h/Î∂?Ï¥??úÍ∏∞)??Ï¥àÎ°ú ?åÏã±
    // Í∑úÏπô: Î¨∏Ïûê???¥Îîî???àÎì† "ÎßàÏ?ÎßâÏúºÎ°??±Ïû•?? ?úÍ∞Ñ???∞ÏÑ† ?¨Ïö©
    parseDurationFromText(text) {
        ...
        // 1) HH:MM(:SS) pattern: use the last match
        try {
        ...
        // 2) Korean/English unit tokens anywhere in the string
        ...
        // 3) Single numeric token with minute/second suffix
    ...
    // ?§Ï†ú ?úÎèô ?ÖÎ†• Î≥ÄÍ≤??? ?çÏä§?∏Ïóê ?¨Ìï®???úÍ∞ÑÍ∞íÏùÑ timer.elapsedÎ°?Î∞òÏòÅ
    syncTimerElapsedFromActualInput(index, text) {
        ...
        // ?Ä?¥Î®∏ ?úÏãú Ï¶âÏãú Í∞±Ïã† (Ï°¥Ïû¨??Í≤ΩÏö∞)
        try {
    ...
    getCurrentTimeIndex() {
        ...
        // 04:00-23:59 map directly from the day-start window.
        if (currentHour >= 4 && currentHour <= 23) {
            return currentHour - 4;
        }
        // 00:00-03:59 wrap to the tail of the 24-slot day.
    ...
    getTimerStartBlockReason(index) {
        ...
        if (timerController && typeof timerController.getStartBlockReason === 'function') {
            return timerController.getStartBlockReason(eligibility, {
                notToday: '?§Îäò ?†Ïßú?êÏÑúÎß??Ä?¥Î®∏Î•??¨Ïö©?????àÏäµ?àÎã§.',
                invalidCurrentSlot: '?ÑÏû¨ ?úÍ∞Ñ ?¨Î°Ø?êÏÑúÎß??úÏûë?????àÏäµ?àÎã§.',
                noPlanned: 'Í≥ÑÌöç???úÎèô???àÏñ¥???Ä?¥Î®∏Î•??úÏûë?????àÏäµ?àÎã§.',
                outOfRange: '?ÑÏû¨ ?úÍ∞Ñ Î≤îÏúÑ??Ïπ∏Ïóê?úÎßå ?Ä?¥Î®∏Î•??úÏûë?????àÏäµ?àÎã§.',
            });
        }

        if (!eligibility.isCurrentDateToday) {
            return '?§Îäò ?†Ïßú?êÏÑúÎß??Ä?¥Î®∏Î•??¨Ïö©?????àÏäµ?àÎã§.';
        }
        if (!Number.isFinite(eligibility.currentIndex) || eligibility.currentIndex < 0) {
            return '?ÑÏû¨ ?úÍ∞Ñ ?¨Î°Ø?êÏÑúÎß??úÏûë?????àÏäµ?àÎã§.';
        }
        if (!eligibility.hasPlannedActivity) {
            return 'Í≥ÑÌöç???úÎèô???àÏñ¥???Ä?¥Î®∏Î•??úÏûë?????àÏäµ?àÎã§.';
        }
        if (!eligibility.isCurrentTimeInRange) {
            return '?ÑÏû¨ ?úÍ∞Ñ Î≤îÏúÑ??Ïπ∏Ïóê?úÎßå ?Ä?¥Î®∏Î•??úÏûë?????àÏäµ?àÎã§.';
        }
        return null;
    }
    ...
    createMergedField(mergeKey, type, index, value) {
        ...
        if (!safeMergeKey) {
            ...
            return `<input type="text" class="input-field ${type}-input"
                           ...
                           placeholder="Í≥ÑÌöç???ÖÎ†•?òÎ†§Î©??¥Î¶≠ ?êÎäî Enter" readonly tabindex="0" aria-label="Í≥ÑÌöç ?úÎèô ?ÖÎ†•" title="?¥Î¶≠?¥ÏÑú Í≥ÑÌöç ?†ÌÉù/?ÖÎ†•" style="cursor: pointer;">`;
        }
        ...
        if (type === 'actual') {
            // ?∞Ï∏° ?§Ï†ú ?úÎèô ?¥Ïùò Í≤ΩÏö∞ ?ÖÎ†• ?ÑÎìú?Ä Î≤ÑÌäº???¨Ìï®?òÎäî Ïª®ÌÖå?¥ÎÑàÎ°?Ï≤òÎ¶¨
            if (index === start) {
            ...
            // Ï¢åÏ∏° Í≥ÑÌöç ?¥ÎèÑ ?àÎ?Î∞∞Ïπò ?§Î≤Ñ?àÏù¥Î°??úÍ∞Å??Î≥ëÌï©, ?àÏù¥?ÑÏõÉ ?†Ï?
            if (index === start) {
                return `<div class="planned-merged-main-container"
                            ...
                            <div class="planned-merged-overlay">
                                <input type="text" class="input-field ${type}-input merged-field merged-main"
                                       ...
                                       placeholder="Í≥ÑÌöç???ÖÎ†•?òÎ†§Î©??¥Î¶≠ ?êÎäî Enter" readonly tabindex="0" aria-label="Î≥ëÌï©??Í≥ÑÌöç ?úÎèô ?ÖÎ†•" title="?¥Î¶≠?¥ÏÑú Í≥ÑÌöç ?†ÌÉù/?ÖÎ†•" style="cursor: pointer;">
                            </div>
                        </div>`;
            } else {
                const isLast = index === end;
    ...
    // Î≥ëÌï©???úÍ∞Ñ?¥Ïùò Ïª®ÌÖêÏ∏??àÏù¥Î∏?Î≤ÑÌäº)Î•?Î≥ëÌï© Î∏îÎ°ù???∏Î°ú Ï§ëÏïô?ºÎ°ú ?¥Îèô
    centerMergedTimeContent(root = document) {
        try {
            ...
            mains.forEach(main => {
                ...
                // Î∏îÎ°ù ?ÑÏ≤¥ ?íÏù¥Î•?Í∞????íÏù¥???©ÏúºÎ°?Í≥ÑÏÇ∞
                let totalHeight = 0;
    ...
    resizeMergedActualContent(root = document) {
        try {
            ...
            mains.forEach((main) => {
                ...
                // ?àÏù¥?ÑÏõÉ?Ä Í≥†Ï†ï, ?úÍ∞Å???∏Í≥Ω???íÏù¥Îß?Î≥Ä?òÎ°ú ?ÑÎã¨
                main.style.setProperty('--merged-actual-block-height', `${totalHeight}px`);
                // ?πÏãú ?®ÏïÑ?àÏùÑ ???àÎäî ?∏Îùº???íÏù¥ ?úÍ±∞
    ...
    // (?òÎèÑ Î≥ÄÍ≤? Ï¢åÏ∏° Í≥ÑÌöç ?ÖÎ†•?Ä Î™®Îã¨Î°úÎßå ?∏Ïßë?òÎ©∞
    // ?∏Ìíã ?ÑÎìú???úÏãú/?†ÌÉù ?©ÎèÑÎ°úÎßå ?¨Ïö©?©Îãà??
    ...
    // ?ÑÏû¨ Ï¢åÌëú ?ÑÏπò???àÎäî type Ïª¨Îüº(.planned-input | .actual-input)???∏Îç±?§Î? Î∞òÌôò
    ...
    // ?†ÌÉù Î∞ïÏä§??Í∏∞Ï? ?¨Í∞Å?ïÏùÑ Ïª¨Îüº/Î≥ëÌï© ?ÅÌÉú??ÎßûÏ∂∞ Î∞òÌôò
    ...
    // Ï¢åÏ∏° ???Ä??ÎßàÏö∞?§Î? ?¨Î†∏?????®Ïùº/Î≥ëÌï© ?Ä?ÅÏùò ?§Ï?Ï§?Î≤ÑÌäº???úÏãú
    ...
    // ?§Ï?Ï§?Î≤ÑÌäº ?∞Ï∏°?ºÎ°ú Î≥ëÌï©/?òÎèåÎ¶¨Í∏∞ Î≤ÑÌäº ?ïÎ†¨
    ...
    openScheduleModal(type, startIndex, endIndex = null) {
        // Modal was removed; route planned edits to the inline dropdown instead.
            ...
            if (shouldRemove) {
                ...
                if (!removed) {
                    ...
                    if (idx >= 0) {
                        ...
                        this.showNotification('?¥Î? ?ôÏùº???ºÎ≤®???àÏñ¥ ?¥Îãπ ?âÏúºÎ°??¥Îèô?àÏñ¥??');
                    } else {
    ...
    editPlannedActivityOption(oldText, newText) {
        ...
        if (i >= 0) {
            // rename in list (?∏Ïßë ?úÏóê????ÉÅ Î°úÏª¨ ??™©?ºÎ°ú Ï∑®Í∏â)
    ...
    makePriorityBadge(rank) {
        ...
        badge.setAttribute('aria-label', `?∞ÏÑ†?úÏúÑ ${rank}`);
        return badge;
    }
    ...
    populateActivityLabelSelect(select, optionLabels, currentLabel, placeholderText = '?∏Î? ?úÎèô') {
        if (!select) return;
        select.innerHTML = '';
    ...
    renderPlannedActivityDropdown() {
        ...
        if (visibleItems.length === 0) {
            ...
            empty.textContent = activeSource === 'notion'
                ? '?∏ÏÖò?êÏÑú Î∂àÎü¨???úÎèô???ÜÏäµ?àÎã§.'
                : 'ÏßÅÏ†ë Ï∂îÍ????úÎèô???ÜÏäµ?àÎã§.';
        ...
        visibleItems.forEach((item) => {
            ...
            if (this.modalPlanSectionOpen) {
                if (hasActivePlanRow) {
                    ...
                    tooltip = '\uC794\uC5EC \uC2DC\uAC04\uC774 \uC5C6\uC5B4 \uCD94\uAC00\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.';
                }
            }
            if (recommendedDisplay) {
                tooltip += ` ¬∑ Ï∂îÏ≤ú ${this.formatDurationSummary(recommendedDisplay)}`;
            }
            li.title = tooltip;
    ...
    handleRoutineMenuAction(action) {
        ...
        if (act === 'pass') {
            ...
            if (!routine) {
                this.showNotification('???úÍ∞Ñ?Ä???§Ï†ï??Î£®Ìã¥???ÜÏäµ?àÎã§.');
                this.closeRoutineMenu();
                return;
            }
        ...
        if (act === 'stop') {
            const routine = ctx.routineAtIndex || ctx.routineForWindow;
            if (!routine) {
                this.showNotification('Ï§ëÎã®??Î£®Ìã¥???ÜÏäµ?àÎã§.');
                this.closeRoutineMenu();
                return;
            }
    ...
    async prefetchNotionActivitiesIfConfigured() {
        ...
        this.setNotionStatus('info', '\uB178\uC158 \uB3D9\uAE30\uD654 \uC900\uBE44 \uC911');
        ...
        try {
            ...
            this.setNotionStatus('success', `?∏ÏÖò ?ôÍ∏∞???ÑÎ£å (${normalized.length}Í∞?`);
    ...
    showNotification(message, type = 'info', options = {}) {
        if (!this.notificationRegion) {
            ...
            region.setAttribute('aria-label', '?åÎ¶º');
        ...
        if (options.actionLabel && typeof options.onAction === 'function') {
            ...
            actionBtn.setAttribute('aria-label', `${options.actionLabel} ?§Ìñâ`);
    ...
    updateActualActivitiesSummary() {
        ...
        if (!Array.isArray(this.modalActualActivities) || this.modalActualActivities.length === 0) {
            if (total > 0) {
                noticeEl.textContent = '?∏Î? ?úÎèô??Ï∂îÍ??òÏÑ∏??';
            }
            return;
        }
        if (total > 0 && used !== total) {
            noticeEl.textContent = '?©Í≥ÑÍ∞Ä ?êÎèô ÎßûÏ∂§?©Îãà??';
        }
    }
...
window.__ttDebug = {
    ...
    seedRunningTimer(index = 0) {
        ...
        tracker.timeSlots[index].planActivities = [{ label: 'Í≥µÎ?', seconds: 3600 }];
        tracker.timeSlots[index].planTitleBandOn = true;
        tracker.timeSlots[index].activityLog = {
            ...(tracker.timeSlots[index].activityLog || {}),
            title: '?òÌîå', details: '', subActivities: [], titleBandOn: true,
