class TimeTracker {
    ...
    collectActualExtraActivities(baseIndex, planUnits) {
        ...
        return existing
            .map((item) => {
                ...
                return { label, seconds, source: extraSource };
            })
            .filter(Boolean);
    }
    ...
    toggleExtraGridUnit(index, extraLabel, unitIndex = null) {
        ...
        const targetUnitIndex = Number.isFinite(unitIndex) ? unitIndex : labelSlots[labelSlots.length - 1];
        if (!labelSlots.includes(targetUnitIndex)) return;

        const nextExtraUnits = currentExtraUnits.slice();
        nextExtraUnits[targetUnitIndex] = !nextExtraUnits[targetUnitIndex];
        ...
        const step = this.getActualDurationStepSeconds();
        const newRecordedUnits = labelSlots.reduce((sum, idx) => sum + (nextExtraUnits[idx] ? 1 : 0), 0);
    ...
    closeScheduleModal() {
        ...
        const planNotice = document.getElementById('planActivitiesNotice');
        if (planNotice) {
            planNotice.textContent = '';
            planNotice.classList.remove('ok');
        }
        const planTotalLabel = document.getElementById('planSplitTotalTime');
        const planUsedLabel = document.getElementById('planSplitUsedTime');
        if (planTotalLabel) planTotalLabel.textContent = '0시간';
        ...
        const planTitleInput = this.ensurePlanTitleButton(document.getElementById('planTitleInput'));
        if (planTitleInput) this.setPlanTitleInputDisplay(planTitleInput, '');
    ...
    saveScheduleFromModal() {
        ...
        if (type && startIndex !== undefined && endIndex !== undefined) {
            if (hasPlanInvalid) {
                alert('계획 분해에 잘못된 시간 형식이 있습니다.');
                return;
            }
            if (isPlanned && sanitizedPlanActivities.length > 0 && planTotalSeconds > 0 && planUsedSeconds !== planTotalSeconds) {
                alert('계획 분해 합계가 총 시간과 일치하지 않습니다.');
                return;
            }

            const planSummary = sanitizedPlanActivities.length > 0 ? this.formatActivitiesSummary(sanitizedPlanActivities) : '';
            const planText = isPlanned ? (planSummary || activity) : activity;

            if (startIndex === endIndex) {
                // 단일 셀
                this.timeSlots[startIndex][type] = planText;
                if (isPlanned) {
                ...
                // 병합된 셀 - 병합 생성 또는 업데이트
                const mergeKey = `${type}-${startIndex}-${endIndex}`;
                this.mergedFields.set(mergeKey, planText);
                
                // 시간대별로 데이터 설정
                for (let i = startIndex; i <= endIndex; i++) {
                    this.timeSlots[i][type] = i === startIndex ? planText : '';
                    if (isPlanned) {
                        if (!this.timeSlots[i].planActivities) this.timeSlots[i].planActivities = [];
    ...
    attachModalEventListeners() {
        ...
        const togglePlanBtn = document.getElementById('togglePlanActivities');
        const planSection = document.getElementById('planActivitiesSection');
        const addPlanRowBtn = document.getElementById('addPlanActivityRow');
        const fillPlanBtn = document.getElementById('fillRemainingPlanActivity');
        const planList = document.getElementById('planActivitiesList');
        const planTitleToggle = document.getElementById('planTitleBandToggle');
        const planTitleInput = this.ensurePlanTitleButton(document.getElementById('planTitleInput'));
        ...
        if (fillPlanBtn) {
            fillPlanBtn.addEventListener('click', () => {
        ...
        const planTitleButton = this.ensurePlanTitleButton(planTitleInput);
        if (planTitleButton) {
            if (planTitleButton.tagName === 'BUTTON') {
                planTitleButton.addEventListener('click', (event) => {
                    event.preventDefault();
                    event.stopPropagation();
                    this.openPlanTitleMenu(planTitleButton, { inline: false });
                });
            } else {
                planTitleButton.addEventListener('input', (event) => {
                    this.modalPlanTitle = event.target.value;
                    this.syncPlanTitleBandToggleState();
                    if (this.renderPlanTitleDropdown) this.renderPlanTitleDropdown();
                });
                planTitleButton.addEventListener('focus', () => {
                    this.lastPlanOptionInput = 'title';
                    if (this.renderPlanTitleDropdown) this.renderPlanTitleDropdown({ open: true });
                });
                planTitleButton.addEventListener('keydown', (event) => {
                    if (event.key === 'Escape') {
                        if (planTitleDropdown) planTitleDropdown.classList.remove('open');
                        return;
                    }
                ...
                planTitleButton.addEventListener('blur', () => {
                    setTimeout(() => {
                        if (planTitleDropdown) planTitleDropdown.classList.remove('open');
                    }, 120);
                });
        ...
        if (clearPlanTitleBtn) {
            clearPlanTitleBtn.addEventListener('click', () => {
                this.modalPlanTitle = '';
                if (planTitleButton) this.setPlanTitleInputDisplay(planTitleButton, '');
                this.modalPlanTitleBandOn = false;
                this.syncPlanTitleBandToggleState();
                if (this.renderPlanTitleDropdown) this.renderPlanTitleDropdown();
            });
        ...
        if (addOptionBtn) {
            addOptionBtn.addEventListener('click', () => {
                ...
                const primaryVal = primaryInput
                    ? (primaryInput === planTitleInput ? this.getPlanTitleInputValue(primaryInput) : this.normalizeActivityText(primaryInput.value))
                    : '';
                const secondaryVal = (!primaryVal && secondaryInput)
                    ? (secondaryInput === planTitleInput ? this.getPlanTitleInputValue(secondaryInput) : this.normalizeActivityText(secondaryInput.value))
                    : '';
                ...
                if (fromTitle) {
                    this.setPlanTitle(value);
                } else if (targetInput && targetInput.tagName !== 'BUTTON') {
                    targetInput.value = '';
                }
            });
        }
    ...
    normalizeActualActivitiesList(raw) {
        ...
        return raw
            ...
            .map(item => {
                ...
                const source = typeof item.source === 'string' ? item.source : null;
                return { label, seconds, source, recordedSeconds };
            })
            .filter(item => item.label || item.seconds > 0);
    }
