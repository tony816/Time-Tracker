...
class TimeTracker {
    ...
    closePlanSegmentMobileTextEditor(options = {}) {
        ...
        if (activeEl && editor.root && editor.root.contains && editor.root.contains(activeEl) && typeof activeEl.blur === 'function') {
            try { activeEl.blur(); } catch (_) {}
        }
        if (editor.root && editor.root.parentNode) {
            editor.root.parentNode.removeChild(editor.root);
        }
        if (editor.backdrop && editor.backdrop.parentNode) {
            editor.backdrop.parentNode.removeChild(editor.backdrop);
        }
        this.mobilePlanSegmentEditor = null;
        if (!this.inlinePlanDropdown && doc && doc.body) {
            doc.body.classList.remove('inline-plan-sheet-open');
        }
        if (options.restoreFocus && editor.labelEl && typeof editor.labelEl.focus === 'function') {
            editor.labelEl.focus();
        }
        return true;
    }
    ...
    shouldUseInlinePlanSheetViewportCorrection() {
        if (typeof this.isInlinePlanMobileInputContext === 'function') {
            return this.isInlinePlanMobileInputContext();
        }
        return this.isCoarsePlanSegmentPointerContext
    ...
    prepareInlinePlanSheetTargetViewport(targetEl) {
        ...
        if (!this.shouldUseInlinePlanSheetViewportCorrection()) return false;
        const root = (typeof window !== 'undefined') ? window : (typeof globalThis !== 'undefined' ? globalThis : null);
        const doc = (typeof document !== 'undefined') ? document : null;
    ...
    applyPlanSegmentResize(baseIndex, segmentIndex, edge, targetMinute) {
        ...
        const resolveMobileEdgeResizeTarget = (event, segmentEl) => {
            ...
            const boundaryRailWidth = Math.min(12, edgeZoneWidth);
            const isOuterBoundaryRail = point.clientX <= rect.left + boundaryRailWidth
                || point.clientX >= rect.right - boundaryRailWidth;
            if (target && target.closest) {
                if (target.closest('.plan-segment-graphic-title, .plan-segment-graphic-label, .plan-segment-label-text, .plan-segment-title-text, .plan-segment-timer-time, .plan-segment-title-edit-input, .activity-chip-board, .inline-plan-dropdown, .inline-plan-subsection, input, select, textarea')) {
                    return null;
                }
                if (target.closest('.plan-segment-timer-button, button') && !isOuterBoundaryRail) {
                    return null;
                }
            }
            if (point.clientX <= rect.left + edgeZoneWidth) return 'left';
        ...
        const startPlanSegmentResizeFromPointer = (event, context) => {
            ...
            if (!resizeSurfaceEl || (segmentEl && segmentEl.classList.contains('is-plan-segment-resize-disabled'))) return false;
            if (!context.allowInteractiveEdge && event.target && event.target.closest && event.target.closest(interactiveResizeBlockSelector)) {
                return false;
            }
            event.preventDefault();
            event.stopPropagation();
            const edge = context && context.edge === 'left' ? 'left' : 'right';
            ...
            if (isPointerEvent && pointerId != null && captureEl && captureEl.setPointerCapture) {
                ...
                const applyResizePreviewGuidePosition = (guide, boundaryMinute, direction) => {
                    ...
                    if (!['left', 'right', 'down', 'up'].includes(direction)) return false;
                    const unitsPerRow = 6;
                    ...
                    const leftPercent = (columnUnit / unitsPerRow) * 100;
                ...
                const appendResizePreviewGuide = (layer, boundaryMinute, options = {}) => {
                    try {
                        ...
                        const direction = ['left', 'right', 'down', 'up'].includes(options.direction)
                            ? options.direction
                            : 'none';
                        if (direction === 'none') return null;
                        const hideLeftArrow = direction === 'right';
                        const hideRightArrow = direction === 'left';
                        const isDownArrow = direction === 'down';
                        const isUpArrow = direction === 'up';
                        const isHorizontalArrow = direction === 'left' || direction === 'right';
                        const insidePositionClass = isHorizontalArrow
                        ...
                        guide.setAttribute('class', `plan-segment-resize-preview-guide plan-segment-resize-preview-arrow${isDownArrow ? ' plan-segment-resize-preview-arrow-down' : ''}${isUpArrow ? ' plan-segment-resize-preview-arrow-up' : ''}${isHorizontalArrow && hideLeftArrow ? ' plan-segment-resize-preview-arrow-right-only' : ''}${isHorizontalArrow && hideRightArrow ? ' plan-segment-resize-preview-arrow-left-only' : ''}${insidePositionClass}`);
                        guide.setAttribute('viewBox', isDownArrow || isUpArrow ? '0 0 40 28' : (hideLeftArrow ? '56 0 40 28' : '0 0 40 28'));
                        guide.setAttribute('width', '40');
                        ...
                        const leftArrow = setAttributes(createSvgElement('path'), {
                            ...
                            d: 'M34.3 6.7H22C20.4 6.7 19.1 5.4 19.1 3.8C19.1 2.5 17.6 0.2 16.6 1L3.1 12.3C1.9 13.3 1.9 15.1 3.1 16.1L16.6 27C17.6 27.8 19.1 25.5 19.1 24.2C19.1 22.6 20.4 21.3 22 21.3H34.3C37.1 21.3 39.3 19.1 39.3 16.3V11.7C39.3 8.9 37.1 6.7 34.3 6.7Z',
                            fill: `url(#${gradientId})`,
                            filter: `url(#${glowId})`,
                        });
                        const rightArrow = setAttributes(createSvgElement('path'), {
                            class: 'plan-segment-resize-preview-arrow-shape',
                            d: 'M61.7 6.7H74C75.6 6.7 76.9 5.4 76.9 3.8C76.9 2.5 78.4 0.2 79.4 1L92.9 12.3C94.1 13.3 94.1 15.1 92.9 16.1L79.4 27C78.4 27.8 76.9 25.5 76.9 24.2C76.9 22.6 75.6 21.3 74 21.3H61.7C58.9 21.3 56.7 19.1 56.7 16.3V11.7C56.7 8.9 58.9 6.7 61.7 6.7Z',
                            fill: `url(#${gradientId})`,
                            filter: `url(#${glowId})`,
                        });
                        ...
                        const upArrow = setAttributes(createSvgElement('path'), {
                            class: 'plan-segment-resize-preview-arrow-shape',
                            d: 'M12.8 4.2V13.4C12.8 15 11.5 16.3 9.9 16.3C8.6 16.3 6.4 17.8 7.2 18.8L18.4 26.1C19.4 27.3 21.2 27.3 22.2 26.1L33.2 18.8C34 17.8 31.8 16.3 30.5 16.3C28.9 16.3 27.6 15 27.6 13.4V4.2C27.6 1.4 25.4 0 22.6 0H17.8C15 0 12.8 1.4 12.8 4.2Z',
                            fill: `url(#${gradientId})`,
                            filter: `url(#${glowId})`,
                        ...
                        if (isDownArrow) {
                            ...
                            guide.appendChild(upArrow);
                        } else if (!hideLeftArrow) {
                            guide.appendChild(leftArrow);
                        }
                        if (isHorizontalArrow && !hideRightArrow) {
                        ...
                        [
                            ...
                            ...(isUpArrow ? [
                                'M18.2 5.6C15.4 11.2 14.6 16.3 15.2 21.9',
                                'M22.8 5.6C25.6 11.2 26.4 16.3 25.8 21.9',
                            ] : []).map(d => ({ d, transform: 'rotate(180 20 14)' })),
                            ...(isHorizontalArrow && !hideLeftArrow ? [
                                'M17 13.8C24.8 11 31.1 10 38.3 10.7',
                                'M14 16.6C24 18.8 32.2 19.4 38.5 17.7',
                            ] : []),
                            ...(isHorizontalArrow && !hideRightArrow ? [
                                'M79 13.8C71.2 11 64.9 10 57.7 10.7',
                                'M82 16.6C72 18.8 63.8 19.4 57.5 17.7',
                            ] : []),
                        ].forEach((entry) => {
                            const sheen = setAttributes(createSvgElement('path'), {
                                class: 'plan-segment-resize-preview-arrow-sheen',
                                d: typeof entry === 'string' ? entry : entry.d,
                            });
                            if (entry && typeof entry !== 'string' && entry.transform) {
                                sheen.setAttribute('transform', entry.transform);
                            }
                            guide.appendChild(sheen);
                        });
                        [
                            ...(isDownArrow ? [
                                ['14.2', '7.8', '0.65'],
                ...
                const updatePreview = (point) => {
                    ...
                    const previewGuideDirection = resolveResizePreviewGuideDirection(resizeTarget, guideTargetPosition);
        ...
        handles.forEach((handle) => {
            ...
            const startResize = (event) => {
                ...
                startPlanSegmentResizeFromPointer(event, { edge, segmentEl, virtualRestEl, handleEl: handle });
            };
