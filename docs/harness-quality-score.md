...
| Actual-lock legacy guard | A | `docs/actual-lock-guardrails.md`, `docs/agent-harness.md` | `npm run test:actual-lock`, `npm test` | Concrete browser scenarios in `docs/actual-lock-guardrails.md` | Effective mask drift, legacy/manual/auto lock confusion, extra allocation into locked units | Browser smoke remains manual |
| Timer | B | `docs/agent-harness.md`, `docs/ai-handoff-map.md` | Timer controller, actual input, time control renderer tests | Browser smoke recommended for rendered control changes | Eligibility, current-slot choice, actual input writes, displayed controls | Need one end-to-end timer smoke checklist |
