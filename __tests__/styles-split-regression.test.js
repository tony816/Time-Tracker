...
test('split css files exist and keep section anchors', () => {
    ...
    assert.match(responsiveSource, /@media \(max-width:\s*768px\), \(hover:\s*none\), \(pointer:\s*coarse\)\s*\{[\s\S]*?\.split-grid-segment\[data-segment-kind="real-plan"\]::before,\s*\.split-grid-segment\[data-segment-kind="real-plan"\]::after\s*\{[^}]*width:\s*min\(var\(--plan-segment-mobile-edge-zone\),\s*42%\);[^}]*pointer-events:\s*none;/s);
    assert.match(responsiveSource, /\.split-grid-segment\[data-segment-kind="real-plan"\]\.is-resizing-plan-segment\.plan-segment-resize-edge-left::before,\s*\.split-grid-segment\[data-segment-kind="real-plan"\]\.is-resizing-plan-segment\.plan-segment-resize-edge-right::after\s*\{[^}]*opacity:\s*0\.86;/s);
