...
test('split css files exist and keep section anchors', () => {
    ...
    assert.match(interactionsSource, /\.plan-segment-resize-handle\s*\{[^}]*width:\s*18px;[^}]*opacity:\s*1;[^}]*pointer-events:\s*auto;/s);
    ...
    assert.match(foundationSource, /\.plan-segment-graphic-main\s*\{[^}]*min-width:\s*0;[^}]*max-width:\s*100%;[^}]*height:\s*100%;[^}]*display:\s*grid;[^}]*grid-template-rows:\s*minmax\(0,\s*1fr\)\s*auto;[^}]*overflow:\s*hidden;/s);
    assert.doesNotMatch(foundationSource, /\.plan-segment-graphic\s*\{[^}]*grid-template-columns:\s*34px\s+minmax\(0,\s*1fr\);/s);
    assert.match(foundationSource, /\.plan-segment-graphic-label\s*\{[^}]*display:\s*inline-flex;[^}]*align-items:\s*center;[^}]*justify-content:\s*center;[^}]*gap:\s*4px;[^}]*align-self:\s*center;/s);
    ...
    assert.doesNotMatch(`${foundationSource}\n${interactionsSource}\n${responsiveSource}`, /durationMinutes\s*<=\s*10|duration-minutes.*compact|compact-plan-segment|plan-segment-compact/);
    ...
    assert.match(responsiveSource, /@media \(max-width:\s*768px\), \(hover:\s*none\), \(pointer:\s*coarse\)\s*\{[\s\S]*?\.plan-segment-boundary-resize-handle-line,\s*\.plan-segment-resize-handle::after\s*\{[^}]*width:\s*5px;[^}]*background:\s*rgba\(30,\s*64,\s*175,\s*0\.74\);/s);
