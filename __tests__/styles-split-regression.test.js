...
test('split css files exist and keep section anchors', () => {
    ...
    assert.match(interactionsSource, /\.plan-segment-boundary-resize-handle-line,\s*\.plan-segment-resize-handle::after\s*\{[^}]*width:\s*24px;[^}]*background:\s*var\(--plan-segment-handle-shell\);[^}]*border:\s*2px solid var\(--plan-segment-handle-stroke\);[^}]*box-shadow:/s);
    assert.match(interactionsSource, /\.plan-segment-boundary-resize-handle-line::after\s*\{[^}]*width:\s*8px;[^}]*background:\s*var\(--plan-segment-handle-color\);[^}]*border:\s*2px solid var\(--plan-segment-handle-stroke\);/s);
    ...
    assert.match(responsiveSource, /@media \(max-width:\s*768px\), \(hover:\s*none\), \(pointer:\s*coarse\)\s*\{[\s\S]*?\.plan-segment-boundary-resize-handle-line,\s*\.plan-segment-resize-handle::after\s*\{[^}]*width:\s*24px;/s);
