...
test('split css files exist and keep section anchors', () => {
    ...
    assert.match(interactionsSource, /\.plan-segment-resize-handle\s*\{[^}]*top:\s*50%;[^}]*width:\s*var\(--plan-segment-handle-hitbox-width\);[^}]*height:\s*min\(\s*var\(--plan-segment-handle-max-height\),\s*calc\(100% - var\(--plan-segment-handle-y-inset\) - var\(--plan-segment-handle-y-inset\)\)\s*\);[^}]*opacity:\s*1;[^}]*--plan-segment-handle-hitbox-width:\s*30px;[^}]*--plan-segment-handle-visual-width:\s*22px;[^}]*--plan-segment-handle-max-height:\s*84px;[^}]*--plan-segment-handle-y-inset:\s*4px;[^}]*--plan-segment-handle-inner-width:\s*8px;[^}]*--plan-segment-handle-inner-y-inset:\s*9px;[^}]*--plan-segment-handle-right-offset:\s*-2px;[^}]*--plan-segment-handle-color:\s*var\(--split-segment-color,\s*rgba\(223,\s*228,\s*234,\s*0\.85\)\);/s);
    assert.match(interactionsSource, /\.plan-segment-resize-handle-right\s*\{[^}]*right:\s*var\(--plan-segment-handle-right-offset\);/s);
    assert.match(interactionsSource, /\.plan-segment-resize-handle-right\.plan-segment-boundary-resize-handle-shared\s*\{[^}]*right:\s*var\(--plan-segment-handle-right-offset\);/s);
    ...
    assert.match(interactionsSource, /\.plan-segment-boundary-resize-handle-line,\s*\.plan-segment-resize-handle::after\s*\{[^}]*width:\s*var\(--plan-segment-handle-visual-width\);[^}]*border-radius:\s*999px;[^}]*background:\s*var\(--plan-segment-handle-shell\);[^}]*border:\s*2px solid var\(--plan-segment-handle-stroke\);[^}]*box-sizing:\s*border-box;[^}]*pointer-events:\s*none;[^}]*box-shadow:/s);
    assert.match(interactionsSource, /\.plan-segment-boundary-resize-handle-line::after\s*\{[^}]*top:\s*var\(--plan-segment-handle-inner-y-inset\);[^}]*bottom:\s*var\(--plan-segment-handle-inner-y-inset\);[^}]*width:\s*var\(--plan-segment-handle-inner-width\);[^}]*border-radius:\s*999px;[^}]*background:\s*var\(--plan-segment-handle-mixed-color\);/s);
    ...
    assert.match(interactionsSource, /\.split-visualization \.split-title-segment,\s*\.split-visualization \.split-grid-segment\s*\{[^}]*z-index:\s*var\(--split-segment-layer,\s*2\);/s);
    assert.match(interactionsSource, /\.split-visualization-planned \.split-grid-segment\.has-shared-plan-boundary-handle\s*\{[^}]*z-index:\s*var\(--split-segment-layer,\s*13\);/s);
    ...
    assert.match(responsiveSource, /@media \(max-width:\s*768px\), \(hover:\s*none\), \(pointer:\s*coarse\)\s*\{[\s\S]*?\.plan-segment-resize-handle\s*\{[^}]*--plan-segment-handle-hitbox-width:\s*40px;[^}]*--plan-segment-handle-right-offset:\s*-7px;[^}]*touch-action:\s*none;/s);
    assert.match(responsiveSource, /@media \(max-width:\s*768px\), \(hover:\s*none\), \(pointer:\s*coarse\)\s*\{[\s\S]*?\.plan-segment-resize-handle-right\s*\{[^}]*right:\s*var\(--plan-segment-handle-right-offset\);/s);
    assert.match(responsiveSource, /@media \(max-width:\s*768px\), \(hover:\s*none\), \(pointer:\s*coarse\)\s*\{[\s\S]*?\.plan-segment-resize-handle-right\.plan-segment-boundary-resize-handle-shared\s*\{[^}]*right:\s*var\(--plan-segment-handle-right-offset\);/s);
    ...
    assert.doesNotMatch(responsiveSource, /@media \(max-width:\s*768px\), \(hover:\s*none\), \(pointer:\s*coarse\)\s*\{[\s\S]*?\.plan-segment-boundary-resize-handle-line,\s*\.plan-segment-resize-handle::after\s*\{[^}]*width:/s);
