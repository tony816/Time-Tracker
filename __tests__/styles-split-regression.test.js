...
test('split css files exist and keep section anchors', () => {
    ...
    assert.match(responsiveSource, /@media \(max-width:\s*768px\), \(hover:\s*none\), \(pointer:\s*coarse\)\s*\{[\s\S]*?\.split-grid-segment\[data-segment-kind="real-plan"\]::before,\s*\.split-grid-segment\[data-segment-kind="real-plan"\]::after\s*\{[^}]*width:\s*36px;[^}]*pointer-events:\s*none;/s);
