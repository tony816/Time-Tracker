...
test('inline add auto-apply on empty slots closes the dropdown', () => {
    assert.match(controllerSource, /if \(canAutoApply\) \{\s+const applyOptions = \{ \.\.\.options, keepOpen: false, keepOpenOnMobile: false \};\s+this\.applyInlinePlanSelection\(val, applyOptions\);\s+\}/);
});
