...
test('mobile time-column CSS contains labels and suppresses obsolete timer box controls', () => {
    ...
    assert.match(responsiveCss, /\.time-entry \.time-slot-container\.merged-time-main::before\s*\{[\s\S]*left:\s*0;[\s\S]*right:\s*0;[\s\S]*height:\s*var\(--merged-block-height, 100%\);[\s\S]*background:\s*#ecf0f1;[\s\S]*border-bottom:\s*2px solid #ccc !important;[\s\S]*box-shadow:\s*inset -2px 0 0 #ddd;[\s\S]*z-index:\s*1;/);
    assert.match(responsiveCss, /\.time-entry\.merge-hover \.time-slot-container\.merged-time-main::before,[\s\S]*\.time-entry\.existing-merged-range \.time-slot-container\.merged-time-main::before\s*\{[\s\S]*border-bottom-color:\s*var\(--merge-outline\) !important;[\s\S]*box-shadow:\s*[\s\S]*inset -2px 0 0 var\(--merge-outline\),[\s\S]*inset 0 2px 0 var\(--merge-outline\);/);
